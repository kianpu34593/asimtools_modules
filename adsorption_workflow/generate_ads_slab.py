...
def generate_ads_slab(
    ...
    def plot_slab(ase_atoms, fig_name):
        ...
        try:
            ...
            index_lst = []
        color_lst = [
            "green" if i != len(vis_atoms) - 1 else "yellow"
            for i in range(len(vis_atoms))
        ]
        for i in index_lst:
            color_lst[i] = "red"
    ...
    def write_to_database(
        ...
        base_slab_db_name = ".".join(
            [reduce_formula, base_slab_facet, "id", idx, placement_mode, "db"]
        )
            ...
            for ads_site, site_dict in results[adsorbate].items():
                for site_coord, site_dict in site_dict.items():
                    ...
                    if visualize:
                        plot_slab(
                            slab_atoms,
                            f"{base_slab.get_chemical_formula(mode='metal')}.{base_slab_facet}.id.{idx}.{adsorbate}.{ads_site}.{site_coord}.{size_str}",
                        )
    ...
    if placement_mode == "all_sites":
        ...
        if ads_slab_image is None:
            raise RuntimeError(
                "`ads_slab_image` cannot be None, if placement_mode is 'most_stable_site'."
            )
        # base_slab_id = int(
        #     ads_slab_image["image_file"].split("/")[-1].split(".id")[1].split(".")[1]
        # )
        ...
        for i, base_slab in enumerate(base_slab_lst):
            ...
            if ads_slab_image is None and adsorbate_xy_lst is None:
                raise RuntimeError(
                    "`ads_slab_image` and `adsorbate_xy_lst` cannot be both None, if placement_mode is 'ads_convergence'."
                )
                ...
                assert len(adsorbate_xy_lst) == len(
                    base_slab_lst
                ), "provided adsorbate_xy_lst length should be the same as the base_slab_lst length."
