...
def generate_sites(
    ...
    slabs_db = connect(slab_image['db_path'])
    if slab_image.get('id') is not None:
        id_num = slab_image.get('id')
        base_slab = slabs_db.get_atoms(id=id_num)
    else:
        raise RuntimeError("database id not specified.")
    ...
    for adsorbate in adsorbate_lst:
        for ads_site, site_dict in results[adsorbate].items():
            for site_coord, site_dict in site_dict.items():
                ...
                if save_to_db:
                    ads_slab_db_name = '.'.join([slab_image['db_path'].split('/')[-1].split('.db')[0],'id',str(id_num),'ads','db'])
                ...
                if visualize:
                    plot_slab(slab_atoms,f"id.{id_num}.{adsorbate}.{ads_site}.{site_coord}") 
