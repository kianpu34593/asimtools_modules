...
def adsorption_energy(
    ...
    if clean_slab_energy is None:
        if isinstance(clean_slab_image["image_file"],dict): 
            adsorbate = ads_slab_image["image_file"].split('ads.')[-1].split('.')[0]
            clean_slab_image["image_file"] = ads_slab_image["image_file"].replace(f"ads.{adsorbate}.","")
        if placement_mode == "most_stable_site" or placement_mode == "ads_convergence":
            clean_slab_image["index"] = ads_slab_image["index"]
        
        results = atom_relax(calc_id=calc_id_slab, image=clean_slab_image, fmax=fmax,  optimizer= optimizer, prefix='clean_slab')
        relax_atoms = get_atoms(image_file=results["files"]["traj"])
        clean_slab_energy = results["energy"]
        if update_db and clean_slab_image["image_file"].split('.')[-1] == 'db':
            path_to_db = clean_slab_image["image_file"]
            with connect(path_to_db) as ads_db:
                ads_db.update(
                    id=int(clean_slab_image["index"]+1),
                    atoms=relax_atoms,
                )
    with open("results.txt", "a") as f:
        print("Clean Slab Energy (eV): ", clean_slab_energy, file=f)
    if adsorbate_energy is None:
        results = atom_relax(calc_id=calc_id_bulk, image=adsorbate_image,  optimizer= optimizer,fmax=fmax)
    ...
    for i, atoms in enumerate(atoms_lst):
        ...
        results = atom_relax(
            calc_id=calc_id_slab, image={"atoms": atoms}, optimizer= optimizer, fmax=fmax, prefix=str(idx)
        )
        ads_slab_energy = results["energy"]
        relax_atoms = get_atoms(image_file=results["files"]["traj"])
        ads_slab_pos = relax_atoms.get_positions()[-1, :]
        ...
        if num_adsorbate_mode == "multiple":
            ...
            raise RuntimeError(
                "`num_adsorbate` needs to be specified. Available options: `multiple` or `single`"
            )
        ads_energy_per_site = np.round(
            (
                ads_slab_energy
                - clean_slab_energy
                - int(num_adsorbate) * adsorbate_energy
            )
            / num_sites,
            decimals=4,
        )
        with open("results.txt", "a") as f:
            ...
            print(
                "Adsorbate conc.: ",
                np.round(int(num_adsorbate) / num_sites, decimals=4),
                file=f,
            )
        if update_db:
            with connect(path_to_db) as ads_db:
